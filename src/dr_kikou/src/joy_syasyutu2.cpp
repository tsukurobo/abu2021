...
//4段階調整、横流し
//モード指定されたら上がっても下がってもその場所に行くように変更
...
int main(int argc, char **argv){
	...
	while(ros::ok()){
		...
		if(mode==1){
			...
			if(subed_enc<(sikii1+100) && subed_enc>(sikii1-100)){
			//しきい値から+-100ならストップ
				pub_msg.data[0]=0;
				pub.publish(pub_msg);
			}	
		...
		if(mode==2){
			...
			if(subed_enc<(sikii2+100) && subed_enc>(sikii2-100)){
			//しきい値から+-100ならストップ
				pub_msg.data[0]=0;
				pub.publish(pub_msg);
			}	
		...
		if(mode==3){
			...
			if(subed_enc<(sikii3+100) && subed_enc>(sikii3-100)){
			//しきい値から+-100ならストップ
				pub_msg.data[0]=0;
				pub.publish(pub_msg);
			}	
		...
		if(mode==4){
			...
			if(subed_enc<(sikii4+100) && subed_enc>(sikii4-100)){
			//しきい値から+-100ならストップ
				pub_msg.data[0]=0;
				pub.publish(pub_msg);
			}	
