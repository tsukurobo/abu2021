...
int motor_rot_vel = {}, enc_now[MOTOR_NUM] = {}, addrs[MOTOR_NUM] = {}; //i2cのアドレス
float wheel_radius = 0.053, goal_vel[MOTOR_NUM]={};
...
void onReceivePower(const abu2021_msgs::motor_pw &); //速度指令(rad/s)が来たら呼び出される関数
...
void setup(){
 ...
 // nh.advertise(debug_pub);
  ...
  {
    nh.logerror("could not get param!");
  }
  ...
  time_prev = micros();
  //明示的に停止させる
  for(int i=0; i<MOTOR_NUM; i++){ if(!(*md[i])==false)*md[i]<<0;}
  //Serial.begin(250000);
}
...
void loop(){
  ...
  if((time_current - time_prev) >= dt*1000000.0 && nh.connected()){
    ...
    for(int i=0; i<MOTOR_NUM; i++){
      ...
      if(!(*md[i]) == false){
        *md[i] >> enc_now[i];
        ...
        // m/sをrad/sに変換
        motor_rot_vel = (int)(goal_vel[i]/wheel_radius);
        //motor_rot_vel = 50;
        if(motor_rot_vel > MAX_ROT_VEL) motor_rot_vel = MAX_ROT_VEL;
        else if(motor_rot_vel < -MAX_ROT_VEL) motor_rot_vel = -MAX_ROT_VEL;

        *md[i] << motor_rot_vel; //モーターを回転させる
      }
      else{
        enc_now[i]=0;
    ...
    enc_msg.e1 = (float)enc_now[0];
    enc_msg.e2 = (float)enc_now[1];
    enc_msg.e3 = (float)enc_now[2];
    enc_msg.e4 = (float)enc_now[3];
    enc_pub.publish(&enc_msg);
    ...
    time_prev = time_current;
     ...
     for(int i=0; i<MOTOR_NUM; i++){ if(!(*md[i])==false)*md[i] << 0;} //ROSに接続されていない間は、停止させる
     time_prev = time_current;
  }
}
